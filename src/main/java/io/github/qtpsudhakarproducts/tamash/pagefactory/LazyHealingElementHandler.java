...
final class LazyHealingElementHandler implements InvocationHandler {
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    try {
      ...
      if (io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.calledFromWait(callSite)
          && !io.github.qtpsudhakarproducts.tamash.healer.HealCache.everHealed(by)
          && io.github.qtpsudhakarproducts.tamash.healer.HealCache.recordFailing(by) <= 3) {
        throw findError;
      }
  ...
  private Object nestedFind(String name, WebElement container, By childBy, Throwable callSite) throws Throwable {
    try {
      ...
      ctx.action = null;
      ctx.kind = "element";
      ctx.description = io.github.qtpsudhakarproducts.tamash.bindings.Bindings
      ...
      ctx.sourceLocation = io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.resolveCallerLocation(callSite);
      ctx.rawVariableName = ctx.description;
      ctx.inAssertion = assertionContext(callSite);
      ctx.inWait = io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.calledFromWait(callSite);
      ctx.frameChain = java.util.List.of();
      io.github.qtpsudhakarproducts.tamash.CurrentTest.Info ti =
          io.github.qtpsudhakarproducts.tamash.CurrentTest.get();
  ...
  private static boolean assertionContext(Throwable callSite) {
    for (var c : io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.resolveConsumerChain(callSite, 3)) {
      if (io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.isAssertionCallSite(c.location())) {
        return true;
      }
    }
    return false;
  ...
  private static boolean negativeOrStrictAssertion(Throwable callSite) {
    for (var c : io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.resolveConsumerChain(callSite, 3)) {
      if (io.github.qtpsudhakarproducts.tamash.bindings.SourceLocations.isNegativeFindContext(c.location(), callSite)) {
        return true;
      }
    }
